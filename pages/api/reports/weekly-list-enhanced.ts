...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  try {
    ...
    // Use Turkish date parsing to handle DD/MM/YYYY format
    ...
    if (weekRanges.length > 0) {
      ...
      // Build a query that returns counts for each date range in one go
      const query = `
        ...
        LEFT JOIN payments ON 
          payments.payment_date >= ranges.start_date AND 
          payments.payment_date <= ranges.end_date
